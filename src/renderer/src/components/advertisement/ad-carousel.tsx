...
export const AdCarousel: React.FC<AdCarouselProps> = memo(({ 
  ...
  const [isAudioMode, setIsAudioMode] = useState(defaultAudioEnabled);
  ...
  const nextAdvertisement = useCallback(() => {
    // 这个函数现在主要用于手动切换（如果需要的话）
    // 自动切换的主要逻辑已经移到 handleVideoEnded 中
      ...
      {isLoading ? (
        ...
        <div className="ad-player-container">
          <video
            ...
            onLoadStart={() => console.log('📺 AdCarousel: 开始加载视频')}
            onCanPlay={() => console.log('📺 AdCarousel: 视频可以播放')}
            onPlay={() => console.log('▶️ AdCarousel: 视频开始播放')}
            onPause={() => console.log('⏸️ AdCarousel: 视频暂停')}
            onTimeUpdate={(e) => {
              const video = e.currentTarget;
              const progress = ((video.currentTime / video.duration) * 100).toFixed(1);
              if (video.currentTime % 5 < 0.1) { // 每5秒打印一次进度
                console.log(`⏰ AdCarousel: 播放进度 ${progress}% (${video.currentTime.toFixed(1)}s/${video.duration.toFixed(1)}s)`);
              }
            }}
            controls
          >
